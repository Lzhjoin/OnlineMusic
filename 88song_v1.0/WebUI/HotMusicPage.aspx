<%@ Page Title="" Language="C#" MasterPageFile="~/Music_MasterPage.master" AutoEventWireup="true" CodeFile="HotMusicPage.aspx.cs" Inherits="NewMusicPage" %>
<%@ OutputCache CacheProfile="Cache60Seconds" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <div class="Detail_ListBorder">
    <div class="title"><span class="Detail_ListFont">热门歌曲</span></div>
    ...
    <%}
      else
      {
        ...
        <span class="Detail_ListName">歌曲名称：<a target="_blank" href="cmp/MusicPlayer_Page.aspx?MusicID=<%=ds_HotMusic.Tables[0].Rows[i]["MusicID"]%>"><%=ds_HotMusic.Tables[0].Rows[i]["MusicName"]%></a>-<a href="DetailSingerPage.aspx?SingerID=<%=ds_HotMusic.Tables[0].Rows[i]["SingerID"]%>"><%=ds_HotMusic.Tables[0].Rows[i]["SingerName"]%></a></span>
        <span class="Detail_ListName">歌曲发布日期：<%=ds_HotMusic.Tables[0].Rows[i]["PublishDate"]%></span>
        </div>
        <%}
      } %>
    </div>
