<%@ Page Title="" Language="C#" MasterPageFile="~/Music_MasterPage.master" AutoEventWireup="true" CodeFile="SingerTypePage.aspx.cs" Inherits="SingerTypePage" %>
<%@ OutputCache CacheProfile="Cache60Seconds" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <div class="Detail_ListBorder">
    <div class="title"><span class="Detail_ListFont">歌手类型-<%=ds_SingerType.Tables[0].Rows[0]["SingerTypeName"] %></span></div>
    <div class="Detail_ListEntrety">
    <%if (ds_SingerType.Tables[1].Rows.Count == 0)
      {%>
        <div class="sky">该类型暂无歌手，请与管理员联系</div>
    <%}
      else
      {
          for (int i = 0; i < ds_SingerType.Tables[1].Rows.Count; i++)
          {%>

        <div class="Detail_List">
        <span class="Detail_ListName">第<%=i + 1%>名：</span>
        <span class="Detail_ListName">歌手名称：<a href="DetailSingerPage.aspx?SingerID=<%=ds_SingerType.Tables[1].Rows[i]["SingerID"] %>"><%=ds_SingerType.Tables[1].Rows[i]["SingerName"]%></a></span>
        <span class="Detail_ListName">歌手点击率：<%=ds_SingerType.Tables[1].Rows[i]["SingerOnclickNum"]%></span>
        </div>
        <%}
      } %>
    </div>
